...
extension NetworkClient {
    ...
    func send(request: NetworkRequest,
              onResponse: @escaping (Result<Data, Error>) -> Void) -> NetworkTask? {
        return send(request: request, completionQueue: .main, onResponse: onResponse)
    }
    ...
    func send<T: Decodable>(request: NetworkRequest,
                            type: T.Type,
                            onResponse: @escaping (Result<T, Error>) -> Void) -> NetworkTask? {
        return send(request: request, type: type, completionQueue: .main, onResponse: onResponse)
    }
}
...
struct DefaultNetworkClient: NetworkClient {
    ...
    private func create(request: NetworkRequest) -> URLRequest? {
        ...
        if let cartRequest = request as? CartRequest {
            let nftsString = cartRequest.nfts.joined(separator: ",")
            let bodyString = "nfts=\(nftsString)&isInCart=\(cartRequest.isInCart)"
